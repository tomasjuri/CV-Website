...
After downloading, simply edit the HTML and CSS files included with `cv` directory. These are the only files you need to worry about, you can ignore everything else! To preview the changes you make to the code, you can open the `index.html` file in your web browser.
...
- `npm run build` builds the project - this builds assets, HTML, JS, and CSS into `cv`
- `npm run build:assets` copies the files in the `src/assets/` directory into `cv`
- `npm run build:pug` compiles the Pug located in the `src/pug/` directory into `cv`
- `npm run build:scripts` brings the `src/js/scripts.js` file into `cv`
- `npm run build:scss` compiles the SCSS files located in the `src/scss/` directory into `cv`
- `npm run clean` deletes the `cv` directory to prepare for rebuilding the project
